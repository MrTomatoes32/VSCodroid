...
android {
    ...
    defaultConfig {
        ...
        ndk {
            abiFilters += supportedAbis.toSet()
        }
...
val verifyPackagedAlignment = tasks.register<Exec>("verifyPackagedAlignment") {
    group = "verification"
    description = "Checks every packaged binary can be mapped and started."
    ...
    failOnExit(
        "A binary in the packaged asset tree cannot be used on Android.\n" +
            ...
            "older.\n" +
            "\n" +
            "PT_INTERP: an executable naming a program interpreter other than the\n" +
            "Android linker came out of an incompatible toolchain and cannot be\n" +
            "started on the device.\n" +
            "\n" +
            "Re-run the script that places the failing file."
    )
}
...
fun jniLibsDir(abi: String): File =
    File(jniLibsRoot, abi)

fun isRealJniBinary(file: File): Boolean =
    file.isFile && file.length() >= jniLibsStubCeiling

fun jniLibsHoldsRealBinary(abi: String): Boolean =
    jniLibsDir(abi).listFiles()
        ?.any(::isRealJniBinary) == true

fun jniLibsHoldsAnyRealBinary(): Boolean =
    supportedAbis.any(::jniLibsHoldsRealBinary)

val verifyBundledBinariesByAbi = supportedAbis.map { abi ->
    val taskName = "verifyBundledBinaries" +
        abi.replace("-", "").replaceFirstChar { it.uppercase() }

    tasks.register<Exec>(taskName) {
        group = "verification"
        description = "Checks bundled $abi binaries can load on Android."
        ...
        commandLine(
            ...
            "--lib-dir", "android/app/src/main/jniLibs/$abi",
        )

        onlyIf { jniLibsHoldsRealBinary(abi) }

        failOnExit(
            "A bundled $abi binary in jniLibs cannot load on Android.\n" +
                "The FAIL line above names the file and the property it fails.\n" +
                "\n" +
                "This ABI is part of the application packaging, so its native\n" +
                "payload must be built for $abi rather than copied from arm64-v8a.\n" +
                "\n" +
                "Re-run the download or build script that owns the failing file."
        )
    }
}

val verifyBundledBinaries = tasks.register("verifyBundledBinaries") {
    group = "verification"
    description = "Checks every bundled binary in every supported ABI can load on Android."
    dependsOn(verifyBundledBinariesByAbi)
    onlyIf { jniLibsHoldsAnyRealBinary() }
}
...
val verifyRequiredBinaries = tasks.register("verifyRequiredBinaries") {
    group = "verification"
    description = "Checks every binary the app resolves by name exists for every supported ABI."
    ...
    doLast {
        val missing = supportedAbis.flatMap { abi ->
            required.filterNot {
                isRealJniBinary(File(jniLibsDir(abi), it))
            }.map {
                "$abi: $it   run ${producers[it]}"
            }
        }

        if (missing.isNotEmpty()) {
            throw GradleException(
                "jniLibs is missing ${missing.size} required native binaries across " +
                    "${supportedAbis.size} supported ABIs:\n" +
                    missing.joinToString("\n") +
                    "\n\nEvery required binary must exist as a real file for every ABI:\n" +
                    supportedAbis.joinToString(", ") +
                    "\n\nA library from arm64-v8a cannot be reused as armeabi-v7a. " +
                    "Each one must be built for its own architecture.\n" +
                    "scripts/build-all.sh runs the producers above in the order that works. " +
                    "scripts/build-aab.sh builds and signs only and packages whatever the " +
                    "last preparation left behind."
            )
        }
    }
}

val verifyBundledShellPathsByAbi = supportedAbis.map { abi ->
    val taskName = "verifyBundledShellPaths" +
        abi.replace("-", "").replaceFirstChar { it.uppercase() }

    tasks.register<Exec>(taskName) {
        group = "verification"
        description = "Checks no bundled $abi binary names Termux's prefix as its shell."

        workingDir = rootProject.projectDir.parentFile
        commandLine(
            "python3", "scripts/patch-default-shell.py",
            "--check", "android/app/src/main/jniLibs/$abi",
        )

        onlyIf { jniLibsHoldsRealBinary(abi) }

        failOnExit(
            "A bundled $abi binary names a shell inside Termux's data directory.\n" +
                ...
                "\n" +
                "Re-run the script that places the file for $abi."
        )
    }
}

val verifyBundledShellPaths = tasks.register("verifyBundledShellPaths") {
    group = "verification"
    description = "Checks no bundled binary in any supported ABI names Termux's prefix as its shell."
    dependsOn(verifyBundledShellPathsByAbi)
    onlyIf { jniLibsHoldsAnyRealBinary() }
}
...
val verifyVenvHome = tasks.register<Exec>("verifyVenvHome") {
    ...
    commandLine(
        ...
        stdlib?.let {
            "android/app/src/main/assets/usr/lib/${it.name}/venv/__init__.py"
        } ?: "missing",
    )
...
fun anyPackHoldsPayload(): Boolean =
    rootProject.projectDir.listFiles()
        ?.any {
            it.isDirectory &&
                it.name.startsWith("toolchain_") &&
                File(it, "src/main/assets/usr").isDirectory
        } == true
...
val packagingGates = listOf(
    checkPatchFingerprints,
    verifyServerTree,
    verifyBundledBinaries,
    verifyRequiredBinaries,
    verifyBundledShellPaths,
    verifyRubyPackShellPaths,
    verifyNativeAddons,
    verifyPackagedAlignment,
    checkPackOverlap,
...
gradle.taskGraph.whenReady {
    ...
    val merging = here.filter {
        it.javaClass.name.contains("MergeSourceSetFolders")
    }
    ...
    val missingJniAbis = supportedAbis.filterNot(::jniLibsHoldsRealBinary)

    if (jniLibsHoldsAnyRealBinary() && missingJniAbis.isNotEmpty()) {
        throw GradleException(
            "This build has native payload for some supported ABIs, but not all of them.\n" +
                "Missing real jniLibs payload for:\n" +
                missingJniAbis.joinToString("\n") { "  $it" } +
                "\n\nSupported ABIs:\n" +
                supportedAbis.joinToString("\n") { "  $it" } +
                "\n\nBoth architectures must be populated before packaging."
        )
    }
}
...
gradle.taskGraph.whenReady {
    val prefix = "${project.path}:"
    val present = allTasks
        .filter { it.path.startsWith(prefix) }
        .map { it.name }
        .toSet()

    val detached = finalizedGates
        .filter { (producer, gate) ->
            producer in present && gate.name !in present
        }
        .map {
            (producer, gate) ->
            "  ${gate.name}, which should follow $producer"
        }
